...
<c:set var="query"
       value="select * from [sont:news] as news where news.[relatedClient]='${currentNode.identifier}' order by news.[date] desc"/>
<jcr:sql var="newsList" sql="${query}" limit="4"/>
<c:forEach items="${newsList.nodes}" var="news">
    <template:module path="${news.path}" view="small" editable="false"/>
</c:forEach>
